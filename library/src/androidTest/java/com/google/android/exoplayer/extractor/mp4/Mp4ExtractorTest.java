 ...
 */
@TargetApi(16)
public class Mp4ExtractorTest extends AndroidTestCase {
    ...
    private static final class Mp4ExtractorWrapper extends Thread {
        ...
        @Override
        public void run() {
            final Uri uri = Uri.parse("file://" + Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM)
                    + "/com.gopro.smarty/GoPro/" + MP4_FILENAME);

            final ExtractorSampleSource source = new ExtractorSampleSource(uri, dataSource,
                    new Mp4Extractor(), 1, 8 * 1024 * 1024);
            Looper.prepare();
            handler = new Handler() {
